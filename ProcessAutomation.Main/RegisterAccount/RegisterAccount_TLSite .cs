...
{
    public class RegisterAccount_TLSite : IRegisterAccount
    {
        ...
        public RegisterAccount_TLSite(RegisterAccountModel data)
        {
            this.data = data;
        }
                        ...
                        case "AddUser":
                            ...
                            if (!webLayout.Url.ToString().Contains(agencies_URL))
                            {
                                ...
                                SendNotificationForError(
                                    ...
                                    $"{Constant.TRUMLANG.ToUpper()} : user { data.WebId + data.IdNumber } lỗi {errorFromCreation}");
                            }
                            else
                            {
                                SendNotificationForError(
                                    $"Tạo user thành công cho web {Constant.TRUMLANG.ToUpper()}",
                                    ...
                                    $"Tên web: {Constant.TRUMLANG.ToUpper()} ({data.WebId})" + Environment.NewLine +
