...
{
    public class RegisterAccount_CTSite : IRegisterAccount
    {
        ...
        private const string web_name = "comthit";
        private const string url = "https://comthit.net/";
        ...
        public RegisterAccount_CTSite(RegisterAccountModel data)
        {
            this.data = data;
        }
                ...
                do
                {
                    switch (process)
                    {
                        ...
                        case "Login":
                            ...
                            if (webLayout.Url.ToString() == index_URL)
                            {
                                if (!Globals.isSentNotification_CT)
                                {
                                    Globals.isSentNotification_CT = true;
                            ...
                            Globals.isSentNotification_CT = false;
                            process = "AccessToDaily";
                            break;
                        case "AccessToDaily":
                            CreateSyncTask();
                        ...
                        case "AddUser":
                            ...
                            if (!webLayout.Url.ToString().Contains(agencies_URL))
                            {
                                ...
                                SendNotificationForError(
                                    ...
                                    $"{Constant.COMTHIT.ToUpper()} : user { data.WebId + data.IdNumber } lỗi {errorFromCreation}");
                            }
                            else
                            {
                                SendNotificationForError(
                                    $"Tạo user thành công cho web {Constant.COMTHIT.ToUpper()}",
                                    ...
                                    $"Tên web: {Constant.COMTHIT.ToUpper()} ({data.WebId})" + Environment.NewLine +
        ...
        private void Login(AdminAccount adminAccount)
        {
            ...
            var otpSetting = adminSetting.Query.Where(x => x.Name == "OTP" && x.Key.ToLower() == Constant.COMTHIT).FirstOrDefault();
