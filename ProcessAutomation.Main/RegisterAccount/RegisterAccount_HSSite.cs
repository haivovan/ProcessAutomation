...
{
    public class RegisterAccount_HSSite : IRegisterAccount
    {
        ...
        private const string web_name = "hoasua";
        private const string url = "https://hoasua.info/";
        ...
        public RegisterAccount_HSSite(RegisterAccountModel data)
        {
            this.data = data;
        }
                ...
                do
                {
                    switch (process)
                    {
                        ...
                        case "Login":
                            ...
                            if (webLayout.Url.ToString() == index_URL)
                            {
                                if (!Globals.isSentNotification_HS)
                                {
                                    Globals.isSentNotification_HS = true;
                            ...
                            Globals.isSentNotification_HS = false;
                            process = "AccessToDaily";
                            break;
                        case "AccessToDaily":
                            CreateSyncTask();
                        ...
                        case "AddUser":
                            ...
                            if (!webLayout.Url.ToString().Contains(agencies_URL))
                            {
                                ...
                                SendNotificationForError(
                                    ...
                                    $"{Constant.HOASUA.ToUpper()} : user { data.WebId + data.IdNumber } lỗi {errorFromCreation}");
                            }
                            else
                            {
                                SendNotificationForError(
                                    $"Tạo user thành công cho web {Constant.HOASUA.ToUpper()}",
                                    ...
                                    $"Tên web: {Constant.HOASUA.ToUpper()} ({data.WebId})" + Environment.NewLine +
        ...
        private void Login(AdminAccount adminAccount)
        {
            ...
            var otpSetting = adminSetting.Query.Where(x => x.Name == "OTP" && x.Key.ToLower() == Constant.HOASUA).FirstOrDefault();
