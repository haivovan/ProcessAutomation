...
{
    public class MMSite : IAutomationPayIn
    {
        ...
        public MMSite(List<Message> data, GeckoWebBrowser web)
        {
            this.data = data;
            this.webLayout = web;
        }
                        ...
                        case "PayIn":
                            ...
                            if (!webLayout.Url.ToString().Contains(agencies_URL))
                            {
                                ...
                                SendNotificationForError(
                                     ...
                                     $"{Constant.MEOMUOP.ToUpper()} : Lỗi + { helper.GetMoneyFormat(currentMessage.Money) } { currentMessage.Web }{ currentMessage.Account } ({ userName })");
                            }
                            else
                            {
                                ...
                                SendNotificationForError(
                                     ...
                                     $"{Constant.MEOMUOP.ToUpper()} : Đã + { helper.GetMoneyFormat(currentMessage.Money) } { currentMessage.Web }{ currentMessage.Account } ({ userName }), SD: { helper.GetMoneyFormat(moneyAfterPay.ToString()) } ");
                            }
        ...
        private AccountData SearchUser()
        {
            ...
            if (userAccount == null || string.IsNullOrEmpty(userAccount.MM))
                return null;
            return userAccount;
        }
        ...
        private void SearchUserClick(AccountData userAccount)
        {
            ...
            userFilter.SetAttribute("value", userAccount.MM);
        ...
        private bool CheckAmoutAccount(decimal currentMoney)
        {
            var minimumMoney = adminSetting.Query.Where(x => x.Name == Constant.MINIMUM_MONEY_NAME
                                                           && x.Key == Constant.MEOMUOP).FirstOrDefault();
            ...
            foreach (GeckoHtmlElement tr in trs)
            {
                ...
                foreach (GeckoHtmlElement td in tds)
                {
                    try
                    {
                        ...
                        if (value != null && value.Trim() == accountData.MM.Trim())
                        {
                            trFound = tr;
                            break;
                        }
        ...
        private void PayIn()
        {
            ...
            var bonus = adminSetting.Query
                    ...
                    .Where(x => x.Key == Constant.MEOMUOP).FirstOrDefault().Value;
