...
{
    public class LQSite : IAutomationPayIn
    {
        ...
        public LQSite(List<Message> data, WebBrowser web)
        {
            this.data = data;
            this.webLayout = web;
        }
                ...
                do
                {
                    switch (process)
                    {
                        ...
                        case "Login":
                            ...
                            {
                                if (!Globals.isSentNotification_LQ)
                                {
                                    Globals.isSentNotification_LQ = true;
                            ...
                            Globals.isSentNotification_LQ = false;
                            process = "CheckAmountAccount";
                        ...
                        case "PayIn":
                            ...
                            if (!webLayout.Url.ToString().Contains(agencies_URL))
                            {
                                ...
                                SendNotificationForError(
                                    ...
                                    $"{Constant.LANQUEPHUONG.ToUpper()} : Lỗi + { helper.GetMoneyFormat(currentMessage.Money) } { currentMessage.Web }{ currentMessage.Account } ({ userName })");
                            }
                            else
                            {
                                ...
                                SendNotificationForError(
                                    ...
                                    $"{Constant.LANQUEPHUONG.ToUpper()} : Đã + { helper.GetMoneyFormat(currentMessage.Money) } { currentMessage.Web }{ currentMessage.Account } ({ userName }), SD: { helper.GetMoneyFormat(moneyAfterPay.ToString()) } ");
                            }
        ...
        private void Login(AdminAccount adminAccount)
        {
            ...
            var otpSetting = adminSetting.Query.Where(x => x.Name == "OTP" && x.Key.ToLower() == Constant.LANQUEPHUONG).FirstOrDefault();
        ...
        private AccountData SearchUser()
        {
            ...
            if (userAccount == null || string.IsNullOrEmpty(userAccount.LQ))
                return null;
            return userAccount;
        }
        ...
        private void SearchUserClick(AccountData userAccount)
        {
            ...
            userFilter.SetAttribute("value", userAccount.LQ.Trim());
        ...
        private HtmlElement FindAccountOnResult(AccountData accountData)
        {
            ...
            foreach (HtmlElement tr in trs)
            {
                ...
                foreach (HtmlElement td in tds)
                {
                    ...
                    {
                        string value = td.InnerText;
                        if (value != null && value.Trim() == accountData.LQ.Trim())
                        {
                            trFound = tr;
                            break;
                        }
        ...
        private void PayIn()
        {
            ...
            var bonus = adminSetting.Query
                   ...
                   .Where(x => x.Key == Constant.LANQUEPHUONG).FirstOrDefault().Value;
