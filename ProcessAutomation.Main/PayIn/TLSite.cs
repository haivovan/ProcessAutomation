...
{
    public class TLSite : IAutomationPayIn
    {
        MailService mailService = new MailService();
        Helper helper = new Helper();
        private GeckoWebBrowser webLayout;
        private List<Message> data = new List<Message>();
        private const string web_name = "trumlang";
        private const string url = "https://trumlang.club/";
        ...
        public TLSite(List<Message> data, GeckoWebBrowser web)
        {
            this.data = data;
            this.webLayout = web;
        }
                ...
                do
                {
                    switch (process)
                    {
                        ...
                        case "Login":
                            ...
                            if (webLayout.Url.ToString() == index_URL)
                            {
                                if (!Globals.isSentNotification_TL)
                                {
                                    Globals.isSentNotification_TL = true;
                            ...
                            Globals.isSentNotification_TL = false;
                            process = "CheckAmountAccount";
                        ...
                        case "PayIn":
                            ...
                            if (!webLayout.Url.ToString().Contains(agencies_URL))
                            {
                                ...
                                SendNotificationForError(
                                     ...
                                     $"{Constant.TRUMLANG.ToUpper()} : Lỗi + { helper.GetMoneyFormat(currentMessage.Money) } { currentMessage.Web }{ currentMessage.Account } ({ userName })");
                            }
                            else
                            {
                                ...
                                SendNotificationForError(
                                     ...
                                     $"{Constant.TRUMLANG.ToUpper()} : Đã + { helper.GetMoneyFormat(currentMessage.Money) } { currentMessage.Web }{ currentMessage.Account } ({ userName }), SD: { helper.GetMoneyFormat(moneyAfterPay.ToString()) } ");
                            }
        ...
        private void Login(AdminAccount adminAccount)
        {
            ...
            var otpSetting = adminSetting.Query.Where(x => x.Name == "OTP" && x.Key.ToLower() == Constant.TRUMLANG).FirstOrDefault();
        ...
        private AccountData SearchUser()
        {
            ...
            if (userAccount == null || string.IsNullOrEmpty(userAccount.TL))
                return null;
            return userAccount;
        }
        ...
        private void SearchUserClick(AccountData userAccount)
        {
            ...
            userFilter.SetAttribute("value", userAccount.TL);
        ...
        private bool CheckAmoutAccount(decimal currentMoney)
        {
            var minimumMoney = adminSetting.Query.Where(x => x.Name == Constant.MINIMUM_MONEY_NAME
                                                           && x.Key == Constant.TRUMLANG).FirstOrDefault();
            ...
            foreach (GeckoHtmlElement tr in trs)
            {
                ...
                foreach (GeckoHtmlElement td in tds)
                {
                    try
                    {
                        ...
                        if (value != null && value.Trim() == accountData.TL.Trim())
                        {
                            trFound = tr;
                            break;
                        }
        ...
        private void PayIn()
        {
            ...
            var bonus = adminSetting.Query
                    ...
                    .Where(x => x.Key == Constant.TRUMLANG).FirstOrDefault().Value;
