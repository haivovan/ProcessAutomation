...
{
    public class DNSite : IAutomationPayIn
    {
        ...
        private const string url = "https://diennuoc.club/";
        ...
        public DNSite(List<Message> data, GeckoWebBrowser web)
        {
            this.data = data;
            this.webLayout = web;
        }
                        ...
                        case "PayIn":
                            ...
                            if (!webLayout.Url.ToString().Contains(agencies_URL))
                            {
                                ...
                                SendNotificationForError(
                                     ...
                                     $"{Constant.DIENNUOC.ToUpper()} : Lỗi + { helper.GetMoneyFormat(currentMessage.Money) } { currentMessage.Web }{ currentMessage.Account } ({ userName })");
                            }
                            else
                            {
                                ...
                                SendNotificationForError(
                                     ...
                                     $"{Constant.DIENNUOC.ToUpper()} : Đã + { helper.GetMoneyFormat(currentMessage.Money) } { currentMessage.Web }{ currentMessage.Account } ({ userName }), SD: { helper.GetMoneyFormat(moneyAfterPay.ToString()) } ");
                            }
        ...
        private AccountData SearchUser()
        {
            ...
            if (userAccount == null || string.IsNullOrEmpty(userAccount.DN))
                return null;
            return userAccount;
        }
        ...
        private void SearchUserClick(AccountData userAccount)
        {
            ...
            userFilter.SetAttribute("value", userAccount.DN);
        ...
        private bool CheckAmoutAccount(decimal currentMoney)
        {
            var minimumMoney = adminSetting.Query.Where(x => x.Name == Constant.MINIMUM_MONEY_NAME
                                                           && x.Key == Constant.DIENNUOC).FirstOrDefault();
            ...
            foreach (GeckoHtmlElement tr in trs)
            {
                ...
                foreach (GeckoHtmlElement td in tds)
                {
                    try
                    {
                        ...
                        if (value != null && value.Trim() == accountData.DN.Trim())
                        {
                            trFound = tr;
                            break;
                        }
        ...
        private void PayIn()
        {
            ...
            var bonus = adminSetting.Query
                    ...
                    .Where(x => x.Key == Constant.DIENNUOC).FirstOrDefault().Value;
